...
%   Parametric-bootstrap likelihood-ratio tests (1,000 samples by default)
%   test site and PET heterogeneity after adjustment for age.
%
...
siteEffectModels = fitGroupingEffectModels(site, age, truth, predictions, ...
    classifierNames, numberOfBootstrapSamples, 'NumberOfSites', ...
    'SiteModelLogLikelihood');
petEffectModels = fitGroupingEffectModels(pet, age, truth, predictions, ...
    classifierNames, numberOfBootstrapSamples, 'NumberOfPETLevels', ...
    'PETModelLogLikelihood');
statisticalNotes = table( ...
    ...
    'Interpretation'}, ...
    {'Two mixed-effects logistic regressions fitted for each classifier: one for site and one for PET'; ...
    'Classification correctness (correct=1, incorrect=0)'; ...
    'Centered continuous age'; ...
    'Separate site and PET random-intercept models'; ...
    'Logistic regression with centered continuous age and no random effect'; ...
    sprintf('%d outcomes simulated under the null model; P values are unavailable if any fit fails', numberOfBootstrapSamples); ...
    'Raw bootstrap P values are adjusted across classifiers within each model using the Holm method'; ...
    'Rows missing age, grouping variable, outcome, or classifier result are excluded'; ...
    'Random-effect variance, ICC, likelihood-ratio statistic, bootstrap P value, and Holm-adjusted P value'; ...
    'A significant result indicates site- or PET-associated heterogeneity after age adjustment, not a causal effect'}, ...
...
writeResults(siteEffectModels, outputFile, 'SiteEffectModels');
writeResults(petEffectModels, outputFile, 'PETEffectModels');
...
function result = fitGroupingEffectModels(groupValues, age, truth, predictions, ...
        classifierNames, numberOfBootstrapSamples, numberOfLevelsName, ...
        modelLogLikelihoodName)
...
groupingLogLikelihood = NaN(numberOfRows, 1);
...
for classifierIndex = 1:numel(classifierNames)
    prediction = predictions(:, classifierIndex);
    valid = ~ismissing(groupValues) & strlength(groupValues) > 0 & ...
        ~isnan(age) & ~isnan(truth) & ~isnan(prediction);
    centeredAge = age(valid) - mean(age(valid));
    modelData = table(categorical(groupValues(valid)), centeredAge, ...
        prediction(valid) == truth(valid), ...
        'VariableNames', {'Grouping', 'AgeCentered', 'Correct'});

    nullModel = fitglm(modelData, 'Correct ~ 1 + AgeCentered', ...
        'Distribution', 'binomial', 'Link', 'logit');
    groupingModel = fitglme(modelData, ...
        'Correct ~ 1 + AgeCentered + (1|Grouping)', ...
        'Distribution', 'Binomial', 'Link', 'Logit', ...
        'FitMethod', 'Laplace');
    covariance = covarianceParameters(groupingModel);
    variance = covariance{1}(1, 1);

    n(classifierIndex) = height(modelData);
    numberOfLevels(classifierIndex) = numel(categories(modelData.Grouping));
    intercept(classifierIndex) = groupingModel.Coefficients.Estimate(1);
    ageCoefficient(classifierIndex) = groupingModel.Coefficients.Estimate(2);
    ...
    groupingLogLikelihood(classifierIndex) = ...
        groupingModel.ModelCriterion.LogLikelihood;
    ...
    for bootstrapIndex = 1:numberOfBootstrapSamples
        ...
        try
            bootstrapNullModel = fitglm(bootstrapData, ...
                ...
                'Distribution', 'binomial', 'Link', 'logit');
            bootstrapGroupingModel = fitglme(bootstrapData, ...
                'Correct ~ 1 + AgeCentered + (1|Grouping)', ...
                'Distribution', 'Binomial', 'Link', 'Logit', ...
                'FitMethod', 'Laplace');
            bootstrapStatistics(bootstrapIndex) = max(0, 2 * ...
                (bootstrapGroupingModel.ModelCriterion.LogLikelihood - ...
...
result = table(classifierOutput, n, numberOfLevels, intercept, ...
    ...
    intraclassCorrelation, nullLogLikelihood, groupingLogLikelihood, ...
    ...
    'VariableNames', {'Classifier', 'N', numberOfLevelsName, ...
    'InterceptLogOdds', 'AgeCoefficientLogOddsPerYear', 'RandomEffectVariance', ...
    'RandomEffectStandardDeviation', 'IntraclassCorrelation', ...
    'NullLogLikelihood', modelLogLikelihoodName, ...
