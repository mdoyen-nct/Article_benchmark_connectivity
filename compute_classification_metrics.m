...
%   tests site heterogeneity without adjustment for age or PET.
%
...
mixedEffectsModels = fitSiteEffectModels(site, truth, predictions, ...
...
statisticalNotes = table( ...
    ...
    'Intercept only; age and PET are not included in the inferential model'; ...
    'Site random intercept'; ...
    'Intercept-only logistic regression with no site random effect'; ...
    ...
    'Rows missing site, outcome, or classifier result are excluded'; ...
    'Site variance, ICC, likelihood-ratio statistic, bootstrap P value, and Holm-adjusted P value'; ...
    'A significant result indicates unadjusted site-associated heterogeneity, not a causal effect'}, ...
...
function result = fitSiteEffectModels(site, truth, predictions, ...
        classifierNames, numberOfBootstrapSamples)
...
for classifierIndex = 1:numel(classifierNames)
    prediction = predictions(:, classifierIndex);
    valid = ~ismissing(site) & strlength(site) > 0 & ...
        ~isnan(truth) & ~isnan(prediction);
    modelData = table(categorical(site(valid)), ...
        prediction(valid) == truth(valid), ...
        'VariableNames', {'Site', 'Correct'});

    nullModel = fitglm(modelData, 'Correct ~ 1', ...
...
result = table(classifierOutput, n, numberOfLevels, intercept, ...
    randomEffectVariance, randomEffectStandardDeviation, ...
