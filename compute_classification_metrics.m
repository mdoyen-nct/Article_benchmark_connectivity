...
mixedEffectsModels = fitMixedEffectsModels(site, pet, ageDecade, truth, ...
    predictions, classifierNames);
statisticalNotes = table( ...
    {'Models'; 'Outcome'; 'FixedEffects'; 'RandomEffects'; 'Age'; ...
    'Inclusion'; 'ReportedResults'; 'Interpretation'}, ...
    {'Three separate mixed-effects logistic regressions fitted for each classifier'; ...
    'Classification correctness (correct=1, incorrect=0)'; ...
    'Intercept only'; ...
    'One random intercept per model: center, PET, or age decade'; ...
    'Age is represented by the same decade categories as the ByAgeDecade sheet'; ...
    'All group sizes are included; only rows missing the outcome or modeled grouping variable are excluded'; ...
    'Random-intercept variance, standard deviation, ICC, log likelihood, AIC, and BIC'; ...
    'Random-effect variance describes heterogeneity and is not a causal effect'}, ...
...
function result = fitMixedEffectsModels(site, pet, ageDecade, truth, ...
        predictions, classifierNames)
groupValues = {site, pet, ageDecade};
modelNames = ["Center", "PET", "AgeDecade"];
numberOfRows = numel(modelNames) * numel(classifierNames);

modelOutput = strings(numberOfRows, 1);
classifierOutput = strings(numberOfRows, 1);
n = zeros(numberOfRows, 1);
...
logLikelihood = NaN(numberOfRows, 1);
aic = NaN(numberOfRows, 1);
bic = NaN(numberOfRows, 1);
...
for classifierIndex = 1:numel(classifierNames)
    ...
    for modelIndex = 1:numel(modelNames)
        row = row + 1;
        groups = groupValues{modelIndex};
        valid = ~ismissing(groups) & strlength(groups) > 0 & ...
            ~isnan(truth) & ~isnan(prediction);
        modelData = table(categorical(groups(valid)), ...
            prediction(valid) == truth(valid), ...
            'VariableNames', {'GroupingVariable', 'Correct'});
        model = fitglme(modelData, ...
            'Correct ~ 1 + (1|GroupingVariable)', ...
            'Distribution', 'Binomial', 'Link', 'Logit');
        covariance = covarianceParameters(model);
        variance = covariance{1}(1, 1);

        modelOutput(row) = modelNames(modelIndex);
        classifierOutput(row) = classifierNames(classifierIndex);
        n(row) = height(modelData);
        numberOfLevels(row) = numel(categories(modelData.GroupingVariable));
        intercept(row) = model.Coefficients.Estimate(1);
        randomEffectVariance(row) = variance;
        randomEffectStandardDeviation(row) = sqrt(variance);
        intraclassCorrelation(row) = variance / (variance + pi^2 / 3);
        logLikelihood(row) = model.ModelCriterion.LogLikelihood;
        aic(row) = model.ModelCriterion.AIC;
        bic(row) = model.ModelCriterion.BIC;
    end
end

result = table(modelOutput, classifierOutput, n, numberOfLevels, ...
    intercept, randomEffectVariance, randomEffectStandardDeviation, ...
    intraclassCorrelation, logLikelihood, aic, bic, ...
    'VariableNames', {'Model', 'Classifier', 'N', 'NumberOfLevels', ...
    'InterceptLogOdds', 'RandomEffectVariance', ...
